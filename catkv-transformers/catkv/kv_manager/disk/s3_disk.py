...
import torch
from catkv.kv_manager.disk.safe_tensor import DiskIOManager

from catkv_ops import S3Manager, S3Schedule


def _default_s3_config_path() -> str:
    release_root = Path(__file__).resolve().parents[4]
    return str(release_root / "csrc" / "config" / "s3.ini")


def _resolve_s3_config_path(config_path: str | None) -> str:
    raw_path = (
        config_path
        or os.environ.get("CATKV_S3_CONFIG")
        or os.environ.get("LMCACHE_CATKV_OPS_S3_CONFIG")
        or _default_s3_config_path()
    )
...
class S3DiskManager(DiskIOManager):
    ...
    def __init__(self, config_path: str | None = None):
        ...
        """
        super().__init__()
        resolved_config_path = _resolve_s3_config_path(config_path)
        self._config_path = resolved_config_path
        self._backend = S3Schedule(resolved_config_path, self.MAX_TASK_THREADS)
        self._manager = S3Manager(resolved_config_path)
        print("S3DiskManager initialized with config:", resolved_config_path)
