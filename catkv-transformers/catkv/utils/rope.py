...
class RotaryEmbeddingESM(torch.nn.Module):
    ...
    def __init__(self, rotary_embedding, head_dim, device='cuda'):
        ...
        self.rotary_dim = (
            rotary_embedding.config.hidden_size
            // rotary_embedding.config.num_attention_heads
        )
        self.device = device
        self.head_dim = head_dim
        self.is_neox_style = True
        self.base = rotary_embedding.config.rope_theta
        self.max_position_embeddings = rotary_embedding.config.max_position_embeddings
        self.max_length = self.max_position_embeddings

        if hasattr(rotary_embedding.config, 'rope_scaling'):
            self.max_length = int(rotary_embedding.config.rope_scaling["factor"] * self.max_length)
            self.yarn_factor = getattr(rotary_embedding.config.rope_scaling, 'factor', 1.0)

        self.position_ids = torch.arange(0, self.max_length, device=self.device)
        self.inv_freq = rotary_embedding.inv_freq.to(self.device)
