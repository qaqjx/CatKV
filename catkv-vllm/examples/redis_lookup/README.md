...
Create a file `lmcache-config.yaml` with Redis configuration:
...
docker run --runtime nvidia --gpus all \
    -v "$(pwd)/lmcache-config.yaml:/config/lmcache-config.yaml" \
    --env "LMCACHE_CONFIG_FILE=/config/lmcache-config.yaml" \
    --env "HF_TOKEN=<YOUR_HUGGINGFACE_TOKEN>" \
    --env "LMCACHE_CHUNK_SIZE=256" \
    --env "LMCACHE_LOCAL_CPU=True" \
    --env "LMCACHE_MAX_LOCAL_CPU_SIZE=5" \
    -v "${HF_CACHE_DIR:-$HOME/.cache/huggingface}:/cache/huggingface" \
